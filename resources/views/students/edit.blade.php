...
@section('content')
    <div class="container">
        <form action="/students/{{$student->id}}" method="post">
            ...
            <div>
                ...
                <select name="group_id">
                    ...
                    @foreach($groups as $group)
                        <option value="{{$group->id}}" @if($group->id == $student->group_id) selected="selected" @endif>{{$group->name}}</option>
                    @endforeach
                </select>
            ...
            <select name="uni_id">
                ...
                @foreach($universities as $university)
                        ...
                        <option value="{{$university->id}}" @if($university->id == $student->uni_id) selected="selected" @endif>{{$university->name}}</option>
                @endforeach
            </select>
            @error('uni_id')
            <span class="invalid-feedback" role="alert">
            ...
            <div>
                ...
                <input id="exp_res"
                       ...
                       name="exp_res"
                       value="{{ old('exp_res') ?? $student->exp_res}}"
                       autocomplete="name" autofocus>
                @error('exp_res')
                <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
            ...
            <textarea id="misc"
                      ...
                      name="misc"
                      autocomplete="name" autofocus>{{$student->misc}}</textarea>
            @error('misc')
            <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
        ...
        <div class="pt-4">
            <a href="/students/{{$student->id}}/delete"><button class="btn btn-primary">Удалить</button></a>
        </div>
    </div>
@endsection
